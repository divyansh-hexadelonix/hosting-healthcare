...
const HostLogin: React.FC = () => {
  ...
  const handleLogin = (e: FormEvent) => {
    ...
    if (foundUser) {
      ...
      localStorage.setItem('hh_host_user', JSON.stringify(hostUser));
      window.dispatchEvent(new Event('hh_host_user_updated'));
      alert(`Login Successful! Welcome back, ${foundUser.name}.`);
      navigate('/dashboard'); 
    } else {
      alert("Invalid credentials. Try david@gmail.com / password123");
    }
  };
        ...
        {screen === 'login' ? (
          ...
          <form className="login-form" onSubmit={handleLogin}>
            ...
            <div className="register-redirect">
              Don't have an account? <Link to = "/host-signup" className="highlight">Register Now</Link>
            </div>
          </form>
        ) : (
          /* Reset Password Form */
